...
export function Navbar(): JSX.Element {
  ...
  return (
    <nav className={`${isMobile ? 'fixed top-0' : ''} bg-gray-900 text-white w-full shadow-lg z-10`}>
        ...
        <Link href="/" className="text-3xl font-bold font-heading text-uiucorange absolute left-5">
          ...
          {isMobile ? (
            <>
              ...
              <div className={`fixed top-16 right-0 h-full bg-gray-900 text-white shadow-lg w-64 transition-transform duration-500 ease-out ${isOpen ? 'transform translate-x-0' : 'transform translate-x-full'}`}>
                <div className="font-custombold flex flex-col px-5 xl:px-12 py-3">
                  <Link href="/" className="mt-4 text-white hover:uiucorange" onClick={closeMenu}>
                    Home
                  </Link>
                  <Link href="/allfood" className="mt-4 text-white hover:uiucorange" onClick={closeMenu}>
                    All Food
                  </Link>
                  <Link href="/" className="mt-4 text-white hover:uiucorange" onClick={closeMenu}>
                    My Favorites
